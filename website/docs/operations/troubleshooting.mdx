...
If a task declares `@requires`, Please fails fast when a tool is missing.
...
If `/please_docs/` works but root does not:

1. Confirm latest deployment uses current `main`.
2. Redeploy docs project.
3. Verify routing config from [`website/vercel.json`](https://github.com/himudigonda/Please/blob/main/website/vercel.json).
