...
| `--theme <name>` | `default` | Dashboard color palette (`default`, `dark`, `light`, `high-contrast`/`hc`). | Also honored via `BROSKI_THEME`. Only meaningful with `--tui` or `broski tui`. |
...
Run `broski tui` with no task to enter the interactive launcher:

- Filterable task list (type-ahead substring match, case-insensitive).
- Free-form input box; `--` in the input forwards trailing tokens as task passthrough args.
- Tab completes the input from the highlighted task; Esc/Ctrl-U clears.
- Enter runs the typed target — or the highlighted match if the input is a partial substring — then pops back to the launcher with a status banner and a recent-runs list.
